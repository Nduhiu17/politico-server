...
from utils.helpers import signup_admin, office_to_post, party_to_post, user, candidate_to_register, signup_user, user_u, \
    ...
    candidate_to_register_non, user33, candidate_to_register_non_voter, vote
...
class TestCandidate(BaseTestCase):
    ...
    def test_posting_with_non_existing_party(self):
        ...
        response = self.client.post('/api/v2/office/1/register', data=json.dumps(candidate_to_register_non_voter),
                                    headers={'Authorization': f'Bearer {result["token"]}',
                                             'Content-Type': 'application' '/json'})
        self.assertEqual(response.status_code, 404)
    ...
    def test_get_results(self):
        ...
        response = self.client.get('/api/v2/office/10/results',headers={'Authorization': f'Bearer {result["token"]}',
                                             'Content-Type': 'application' '/json'})
        self.assertEqual(response.status_code, 404)
        response = self.client.post('/api/v2/offices', data=json.dumps(office_to_post),
                                    headers={'Authorization': f'Bearer {result["token"]}',
        ...
        response = self.client.post('/api/v2/votes', data=json.dumps(vote),
                                    headers={'Authorization': f'Bearer {result["token"]}',
                                             'Content-Type': 'application' '/json'})
        self.assertEqual(response.status_code, 201)
        response = self.client.get('/api/v2/office/1/results',headers={'Authorization': f'Bearer {result["token"]}',
                                             'Content-Type': 'application' '/json'})
